...
def normalize_features(results):
    ...
    def standardize(landmarks): return (landmarks - chest_center) / shoulder_width

    return np.concatenate([standardize(pose).flatten(), standardize(lh).flatten(), standardize(rh).flatten()])

try: Interpreter = tf.lite.Interpreter
except: from tflite_runtime.interpreter import Interpreter
